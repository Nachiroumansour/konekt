  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch('/api/auth/register', {
        ...
        body: JSON.stringify({ phone: formattedPhone, email, password }),
      });
      ...
      <div className="max-w-md w-full space-y-8 bg-white p-10 rounded-2xl shadow-xl border border-slate-100">
        ...
        <form className="mt-8 space-y-6" onSubmit={handleSubmit}>
          <div className="space-y-4">
            ...
            <div>
              <label htmlFor="email" className="block text-sm font-medium text-slate-700 mb-1">Email (Optionnel)</label>
              <div className="relative rounded-md shadow-sm">
                <input
                  type="email"
                  id="email"
          ...
          <div className="text-center mt-4">
            <p className="text-sm text-slate-600">
              Déjà un compte ? <Link to="/login" className="font-medium text-emerald-600 hover:text-emerald-500">Connectez-vous</Link>
            </p>
          </div>
        </form>
      </div>
